...
export default function createCopyOnWriteState<T extends object>(baseState: T): IStore<T> {
    let updateState: ((...argument) => any) | null = null;
    ...
    class CopyOnWriteStoreProvider extends React.Component<any> {
        ...
        render() {
            return <State.Provider value={this.state}>{this.props.children}</State.Provider>;
        }
    }

    class ConsumerMemoization extends React.Component<{ state; version; children }> {
        public shouldComponentUpdate({ state, version }) {
            const currentState = this.props.state;
            return (
                version !== this.props.version ||
                state.some((observedState, i) => !isEqual(observedState, currentState[i]))
            );
        }
    ...
    class CopyOnWriteConsumer extends React.Component<any> {
        static defaultProps = {
            select: [identityFn],
            consume: null,
        };
